...
xmin, xmax = -bd,bd
ymin, ymax = -bd,bd
funcName = "Fixed stepsize J-symm"
max_it = 50
...
x_sol, y_sol,iter, nfs, val, ng,ZZ  = secant_inv([-4.0],[-2.0],obj,stepsize,dummy,max_it,prt,0,F_tol)
...
x_sol, y_sol,iter, nfs, val, ng,ZZ  = secant_inv([-4.0],[0.0],obj,stepsize,dummy,max_it,prt,0,F_tol)
plot!( ZZ[1,:],ZZ[2,:], marker = (:circle, 4))
F_sol = [obj.∇xL(x_sol,y_sol); -obj.∇yL(x_sol,y_sol)]
nabla_F_sol  = obj.∇F(x_sol,y_sol);
g_sol = nabla_F_sol'*F_sol;
...
x_sol, y_sol,iter, nfs, val, ng,ZZ  = secant_inv([-2.0],[-4.0],obj,stepsize,dummy,max_it,prt,0,F_tol)
...
x_sol, y_sol,iter, nfs, val, ng,ZZ  = secant_inv([0.0],[-4.0],obj,stepsize,dummy,max_it,prt,0,F_tol)
plot!( ZZ[1,:],ZZ[2,:], marker = (:circle, 4))
F_sol = [obj.∇xL(x_sol,y_sol); -obj.∇yL(x_sol,y_sol)]
nabla_F_sol  = obj.∇F(x_sol,y_sol);
g_sol = nabla_F_sol'*F_sol;
...
#plot!( title = string(funcName, ", ", TYPE,", A = $interaction, fixed stepsize=$stepsize "), titlefont=font(8), legend = false,aspect_ratio=:equal )# , xlims=(xmin,xmax), ylims=(ymin,ymax))
plot!(
...
savefig("plots/$prob_type-NOLS_Sec-$interaction.png")
